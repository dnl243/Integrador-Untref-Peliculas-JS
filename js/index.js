...
frMovie();
  function frMovie() {
    ...
    <div class="card mb-3 bg-transparent border-0 w-75">
      <div class="row g-0">
        <div class="col-sm-4">
          <a href="./detalle.html?codigo=${moviesData.results[0].id}"><img src="https://image.tmdb.org/t/p/w500${moviesData.results[0].poster_path}" alt="${moviesData.results[0].title}" class="img-fluid rounded-4"></a>
        </div>
        <div class="col-sm-8">
          <div class="card-body cont-banner">
            <h2 class="card-title banner-title">${moviesData.results[0].title}</h2>
            <p class="card-text banner-desc h5">${moviesData.results[0].overview}</p>
            <a href="./detalle.html?codigo=${moviesData.results[0].id}" class="btn btn-dark">Detalle</a>
          </div>
        </div>
      </div>
    </div>`;
